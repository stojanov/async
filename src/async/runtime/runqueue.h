...
class runqueue {
  public:
    using task_object = std::variant<task_package, coro_handle>;
    ...
    void push_pending_task(T &&task) {
        _pending_raw_tasks.emplace_back(std::move(task));
    }
    ...
    void activate(const task_package &task) {
        bool expected = true;
        ...
        const auto visitor =
            var_overload{[](void_func func) { func(); },
                         [&](s_ptr<pending_coro_base> pending_coro) {
                             auto id = _coro_id.get();

                             auto handle = pending_coro->construct(id);

                             std::lock_guard lck(_coroM);
                             _coroutines.insert(std::pair{
                                 id, coro_block{id, handle, pending_coro}});
  ...
  private:
    ...
    std::deque<task_package> _pending_raw_tasks;
